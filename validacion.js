...
function validaDNI() {
    ...
    if (!elemento.checkValidity()) {
        if (elemento.validity.valueMissing) {
            error2(elemento, "Debe introducir un número valido")
        }
        if (elemento.validity.patternMismatch) {
            error2(elemento, "El número de DNI debe tener 8 numeros");
        } //error(elemento);
        return false;
    }
    return true;
...
function validar(e) {
    borrarError();
    if (validaNombre() && validaEdad() && validaDNI() && confirm("Pulsa aceptar si deseas enviar el formulario")) {
        return true
    } else {
        e.preventDefault();
        return false;
